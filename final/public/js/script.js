...
$(function() {
  function resetTitleEllipsisWidth() {
    $('.board-table .title-text').each(function(i, e) {
      ...
      var $comment2 = $(e).closest('.title-container').find('.title-comments2');
      var $comment3 = $(e).closest('.title-container').find('.title-comments3');
      var textWidth = $text.width();
      var ellipsisWidth = $ellipsis.outerWidth();
      var commentWidth = $comment.outerWidth();
      var comment2Width = $comment2.outerWidth();
      var comment3Width = $comment3.outerWidth();
      var padding = 1;
      ...
      if (ellipsisWidth <= (textWidth + comment2Width + padding)) {
        $ellipsis.width(ellipsisWidth - (comment2Width + padding));
      } else {
        $ellipsis.width(textWidth + padding);
      }
      if (ellipsisWidth <= (textWidth + comment3Width + padding)) {
        $ellipsis.width(ellipsisWidth - (comment3Width + padding));
      } else {
        $ellipsis.width(textWidth + padding);
      }
    });
